...
def main(unused_argv):
    ...
    import random
    i = random.randint(0, len(query_list))  # 1부터 100 사이의 임의의 정수
    ...
    rand_image = query_list[i]
    rand_class = query_list[i][:-8]
    print('sample_query_name:', rand_image)
    sample_query_index = query_list.index(rand_image)
    print('sample_query_index:', sample_query_index)
    pil_img = Image(filename='/workspace/Yonsei_Dataset/query/' + rand_class + '/' + rand_image)
    print('/workspace/Yonsei_Dataset/query/' + rand_class + '/' + rand_image)
    display(pil_img)
    
    print('Top 2:', result[i][:2])
    for j in result[i][:2]:
        print(index_list[j])        
        im = Image(filename='/workspace/Yonsei_Dataset/index/' + index_list[j][:-8] + '/' + index_list[j])
        display(im)
    ...
    print('mAP', ComputeMetrics(result[sample_query_index:sample_query_index+1], ground_truth[sample_query_index:sample_query_index+1], query_list[sample_query_index:sample_query_index+1], _PR_RANKS, 10, start = mAP_start))
    # print('mAP', ComputeMetrics(result, ground_truth, query_sample, _PR_RANKS, 15, start = mAP_start))
    # print('mAP', ComputeMetrics(result, ground_truth, query_sample, _PR_RANKS, 20, start = mAP_start))
    # print('mAP', ComputeMetrics(result, ground_truth, query_sample, _PR_RANKS, 50, start = mAP_start))
    # print('mAP', ComputeMetrics(result, ground_truth, query_sample, _PR_RANKS, 100, start = mAP_start))

    query_sample.append(query_list[i])
    
    # Image(filename='/workspace/Yonsei_Dataset/query/' + rand_class + '/' + rand_image)
    img = Image.open('/workspace/Yonsei_Dataset/query/' + rand_class + '/' + rand_image)
    img.show() 
