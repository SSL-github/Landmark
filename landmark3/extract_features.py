...
def main(unused_argv):
  # Read list of images.
  f = open(cmd_args.list_images_path, 'w')
  folder_list = sorted(os.listdir(cmd_args.image_folder_location))  
  for folder in folder_list :
     temp_image_folder_location = str(cmd_args.image_folder_location) + str(folder)
     image_list = os.listdir(temp_image_folder_location)
#     print(image_list)
     image_list=sorted(image_list)
     for image_name in image_list :
        if image_name != '.ipynb_checkpoints':
            image_location = str(cmd_args.image_folder_location) + str(folder) + '/' + str(image_name)+ '\n'
            f.write(image_location)
  f.close()
  ...
  for i in tqdm(range(num_images)):
    ...
    descriptors_out = np.mean(descriptors_out,axis=0)
    np.save(out_desc_fullpath,descriptors_out)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      type=str,
      default='/home/smart02/Yonsei_Dataset/query/',
      help="""
