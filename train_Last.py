...
flags.DEFINE_string('train_file_pattern', '/home/smart02/Final_dataset/resized_train_tfrecord/train*',
                    'File pattern of training dataset files.')
flags.DEFINE_string('validation_file_pattern', '/home/smart02/Final_dataset/resized_validation_tfrecord/validation*',
                    'File pattern of validation dataset files.')
flags.DEFINE_integer('num_class', 2503, 'number of classes.')
# flags.DEFINE_integer('seed', 0, 'Seed to training dataset.')
flags.DEFINE_float('initial_lr', 0.01, 'Initial learning rate.')
flags.DEFINE_integer('batch_size', 16, 'Global batch size.')
flags.DEFINE_integer('max_iters',700000, 'Maximum iterations.')
...
#     print('Number of devices:', strategy.num_replicas_in_sync)
  ...
  image_size = 321
#valid_data 수
  num_eval_batches = int(80309 / global_batch_size)
    #100 1000 20000
  report_interval = 10
  eval_interval = 10000
  save_interval = 10000
  ...
  validation_dist_dataset = strategy.experimental_distribute_dataset(
      validation_dataset)
#   print('train_dist_dataset:', train_dist_dataset)
#   print('validation_dist_dataset:', validation_dist_dataset)
...
#   print('next(train_iter):', next(train_iter)[0].shape)
#   print('next(validation_iter):', next(validation_iter))

#   print('input_batch == next(train_iter)  shape: ', input_batch.shape)
  ...
  with strategy.scope():
    ...
    model = create_model(num_classes)  # DELF
    ...
    # Setup checkpoint directory. (previous version)
#     checkpoint = tf.train.Checkpoint(optimizer=optimizer, model=model)
#     manager = tf.train.CheckpointManager(
#         checkpoint, checkpoint_prefix, max_to_keep=3)
    tr = tracking.AutoTrackable()
    checkpoint = tf.train.Checkpoint(optimizer=optimizer, model=model)
    manager = tf.train.CheckpointManager(
        checkpoint, checkpoint_prefix, max_to_keep=3)
    ...
    def train_step(inputs):
      ...
      with tf.GradientTape() as gradient_tape:
        ...
        print('attn_pre',attn_prelogits)
        print('attn_pre_size',attn_prelogits.shape)
