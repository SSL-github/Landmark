...
#     """
#     image_paths = tf.io.gfile.glob(os.path.join(image_dir, '**************_***.JPG'))
...
def _get_train_image_files_and_labels(csv_path, image_dir):
...
def _build_train_and_validation_splits(image_paths, file_ids, labels, validation_split_size, seed):
    ...
    # which are 'str'.
    labels_str = [str(label) for label in labels] # label을 string으로 변경
    image_attrs = np.stack((image_paths, file_ids, labels_str)) # (3, 이미지 개수)로 한 row당 image_paths, file_ids, labels_str 쌓음.
    image_attrs_idx_by_label = {} # key가 label이고, value는 []로 구성되어 몇 번째 이미지가 해당 label에 포함되는지를 나타냄.
    ...
    # validation split.
    # _VALIDATION_SPLIT과 _TRAIN_SPLIT 각각 image_paths, file_ids, labels_str 세트로 들어감.
    splits = {
            _VALIDATION_SPLIT: [],
            _TRAIN_SPLIT: []
    }
    ...
    for label, indexes in image_attrs_idx_by_label.items():
        ...
        image_attrs_label = image_attrs[:, indexes] # [[image_paths, file_ids, labels_str]]로 레이블당 구성됨.
        # Shuffle the current label subset.
        image_attrs_label = _shuffle_by_columns(image_attrs_label, rs) # column을 셔플함.
        # Split the current label subset into TRAIN and VALIDATION splits and add
        # each split to the list of all splits.
        images_per_label = image_attrs_label.shape[1] # label 당 이미지 수
...
def _build_train_tfrecord_dataset(csv_path, image_dir, 
                                  generate_train_validation_splits, validation_split_size, 
                                  seed):
    ...
    image_paths, file_ids, labels = _get_train_image_files_and_labels(csv_path, image_dir)
